...
class SyncRoles(commands.Cog):
    ...
    async def get_target_servers(self, source_guild):
        ...
        logger.debug(f"Getting target servers for guild ID: {source_guild.id}")
        ...
        logger.debug(f"Found target servers: {[g.id if g else None for g in targets]}")
        return targets
    ...
    async def get_member_in_guild(self, member_id, guild):
        ...
        try:
            ...
            logger.debug(f"Member {member_id} not found in guild {guild.id}")
            return None
        except discord.Forbidden:
            logger.warning(f"Missing permissions to fetch members in guild {guild.id}")
            return None
        except discord.HTTPException as e:
            logger.error(f"Error fetching member {member_id} in guild {guild.id}: {e}")
            return None
    ...
    async def sync_roles(self, member, source_guild):
        ...
        if not target_guild1 or not target_guild2:
            logger.warning("Could not find both target guilds")
            return
        ...
        if not source_server:
            logger.warning(f"Unknown source guild ID: {source_guild.id}")
            return
        
        logger.debug(f"Source server identified as: {source_server}")
        ...
        for role_name, role_id in ROLE_MAPPING[source_server].items():
            if role_id and role_id in source_roles:
                ...
                logger.debug(f"Found mapped role: {role_name} (ID: {role_id})")
        
        logger.info(f"Roles to sync: {roles_to_add}")
        
        # Sync to target servers
        for target_member, target_guild in [(target_member1, target_guild1), (target_member2, target_guild2)]:
            if not target_member:
                logger.debug(f"Skipping guild {target_guild.id} - member not found")
                continue
            ...
            if not target_server:
                logger.warning(f"Unknown target guild ID: {target_guild.id}")
                continue
            
            logger.debug(f"Syncing to target server: {target_server}")
            ...
            for role_name, role_id in ROLE_MAPPING[target_server].items():
                ...
                if role_name not in roles_to_add and int(role_id) in current_roles:
                    logger.debug(f"Removing role: {role_name} (ID: {role_id})")
            ...
            for role_name in roles_to_add:
                ...
                if not role_id:  # Skip if role doesn't exist in target server
                    logger.debug(f"Skipping {role_name} - no mapping in target server")
                    continue
                
                if int(role_id) not in current_roles:
                    logger.debug(f"Adding role: {role_name} (ID: {role_id})")
            ...
            for role_id in new_roles:
                ...
                if role:
                    ...
                    logger.warning(f"Role ID {role_id} not found in guild {target_guild.id}")
            
            try:
                await target_member.edit(roles=new_role_objects)
                logger.info(f"Successfully synced roles for {target_member} in {target_server}")
            except discord.Forbidden:
    ...
    async def on_member_update(self, before, after):
        ...
        if before.roles != after.roles:
            logger.info(f"Member roles updated: {after} in guild {after.guild.id}")
            logger.debug(f"Before roles: {[r.id for r in before.roles]}")
            logger.debug(f"After roles: {[r.id for r in after.roles]}")
    ...
    async def force_sync(self, ctx, member: discord.Member):
        ...
        logger.info(f"Force sync initiated by {ctx.author} for {member}")
